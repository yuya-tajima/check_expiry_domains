...
/**
 * 指定されたドメインの有効期限をチェックする
 *
 * @param array $args {
 *
 *     Optional.
 *     @type array $excludes    除外するドメインの配列
 * }
 * @return void
 */
function exe_check_expiry_domains( $args = array() ) {
        ...
        if( $errno = curl_errno( $ch ) ) {
            ...
            if ( $response ) {
                ...
                foreach ( $expires as $e ) {
                    ...
                    $interval        = $today_obj->diff( $expire_date_obj );
                    $expire_days     = (int) $interval->format('%r%a');
                    if ( $expire_days < 0 ) {
                        $expired[$service][] = [ $e[0], $e[1], $expire_days ];
                    } elseif ( $alert_interval > $expire_days ) {
    ...
    if ( $expired ) {
        $buf = array();
        $buf[] = sprintf( 'ERROR: The expiry date has already passed.' );
        $exist = false;
        foreach ( $expired as $service => $_expires ) {
            $buf[] = sprintf( '### Domain service is %s ###', $service );
            foreach ( $_expires as $e ) {
                if ( in_array( $e[0], $args['excludes'] ) ) {
                    continue;
                }
                $exist = true;
                $buf[] = sprintf( '%s is expired domain. expiry date is %s.', $e[0], $e[1] );
            }
        }
        if ( $exist ) {
            fwrite( STDOUT, implode( PHP_EOL, $buf ) );
    ...
    if ( $soon_expires ) {
        $buf = array();
        $buf[] = sprintf( 'WARNING: The expiry date is approaching.' );
        $exist = false;
        foreach ( $soon_expires as $service => $_soon_expires ) {
            ...
            foreach ( $_soon_expires as $e ) {
                ...
                $exist = true;
                $buf[] = sprintf( '%s will expire in %d days. expiry date is %s.', $e[0], $e[2], $e[1] );
            }
        }
        if ( $exist ) {
            fwrite( STDOUT, implode( PHP_EOL, $buf ) );
    ...
    if ( $late_expires ) {
        $buf = array();
        $buf[] = sprintf( 'INFO: There are over %d days until the expiry date.', $alert_interval );
        $exist = false;
...
if ( ! empty( $argv[1] ) ) {
    if ( ! is_file( $argv[1] ) ) {
        fwrite( STDERR, sprintf( '%s does not exist.', $argv[1] ) . PHP_EOL );
        exit(1);
    }
    $domains = file( $argv[1], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES );
    $args['excludes'] = $domains;
}

exe_check_expiry_domains( $args );
